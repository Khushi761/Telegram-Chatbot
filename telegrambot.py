...
class TelegramBot(object):
    def __init__(self,token):
        ...
        self.last_id = None             #to get new messages and not old+new
    
    def delete_webhook(self):       #telegram won't be able to send new messages 
        try:                        #try and except (like if statement) for errors
            return requests.get(f"(self.BOT_URL)deletewebhook").json() #return gives response from what is ahead to work with
        except requests.exceptions.ConnectionError:
            return self.no_network()

    def set_webhook(self, webhook):
        try:
            return requests.post(f"(self.BOT_URL)setWebhook", json = {"url":webhook})
        except requests.exceptions.ConnectionError:
            return self.no_network()
